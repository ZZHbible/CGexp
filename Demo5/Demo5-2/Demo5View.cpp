...
vector<vector<Point3> > bezierSurfaceCtls;// bezier surface control points. 4 by 4. actually, surface point should has three elements, not two
...
void CDemo5View::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (-1 == capturedPtId[0])
		return;
	if (-1 == capturedPtId[0] && -1 != capturedPtId[1])
		return; // actually, there must be some problem occurs now.
